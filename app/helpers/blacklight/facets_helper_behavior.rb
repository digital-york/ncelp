...
module Blacklight::FacetsHelperBehavior
  ...
  # OASIS modified facet behaviour to retrieve label from id
  # override render_facet_item method to read facet labels from from authority service
  def get_model_field_name(solr_name)
    n = solr_name.dup
    solr_appendix = ["_sim","_tesim"]
    solr_appendix.each do |a|
      n.slice! a
    end
    n
  end
  ...
  def render_facet_item(facet_field, item)
    ...
    if NcelpAuthorities.authority_list.include? f and item.value != ''
      # seek authority label and update facet_label
      facet_label = GenericLocalAuthorityService.id_to_label(f,item.value)
