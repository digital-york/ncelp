...
module Blacklight::RenderConstraintsHelperBehavior
  ...
  def render_filter_element(facet, values, path)
    ...
    safe_join(Array(values).map do |val|
      ...
      # OASIS changes to make the facet filter on top working
      current_label = val.dup
      f = get_model_field_name(facet)
      if NcelpAuthorities.authority_list.include? f and val != ''
        facet_label = GenericLocalAuthorityService.id_to_label(f,val)
        current_label = facet_label if facet_label!=''
      end

      render_constraint_element(facet_field_label(facet_config.key),
                                facet_display_value(facet, current_label),
